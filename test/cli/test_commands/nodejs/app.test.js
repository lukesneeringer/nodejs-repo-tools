...
test.serial('should do a dry run build with defaults', async (t) => {
  ...
  t.regex(output, new RegExp(`build: Detected repository: /GoogleCloudPlatform/nodejs-repo-tools`));
  t.regex(output, new RegExp(`build: Detected SHA:`));
  if (process.env.GOOGLE_APPLICATION_CREDENTIALS) {
    t.regex(output, new RegExp(`build: Copying: ${path.resolve(process.env.GOOGLE_APPLICATION_CREDENTIALS)}`));
  }
  if (process.env.GCLOUD_PROJECT || process.env.GOOGLE_CLOUD_PROJECT) {
    t.regex(output, new RegExp(`build: Setting build project ID to: ${process.env.GCLOUD_PROJECT || process.env.GOOGLE_CLOUD_PROJECT}`));
  }
  t.regex(output, new RegExp(`build: Compiling: ${path.join(samplePath, 'repo-tools-cloudbuild.yaml')}`));
  t.regex(output, new RegExp(`build: Printing: ${path.join(samplePath, 'repo-tools-cloudbuild.yaml')}`));
  ...
  t.regex(output, new RegExp(`    'REPO_PATH=/GoogleCloudPlatform/nodejs-repo-tools',`));
  t.regex(output, new RegExp(`    'CI=`));
  t.regex(output, new RegExp(`    'CONTEXT=test-samples-nodejs-app',`));
  t.regex(output, new RegExp(`    'GOOGLE_APPLICATION_CREDENTIALS=${keyFilePath || ''}',`));
  t.regex(output, new RegExp(`    'GCLOUD_PROJECT=${process.env.GCLOUD_PROJECT || process.env.GOOGLE_CLOUD_PROJECT || ''}',`));
